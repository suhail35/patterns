"""
Стратегия (Strategy).
Этот шаблон необходим для того, чтобы определять семейства алгоритмов и инкапсуляции каждого из них.
Кроме того, данный паттерн позволяет обеспечить их взаимозаменяемость.
Благодаря этому появляется возможность подбора алгоритма посредством определения соответствующего класса.
С помощью Strategy можно изменять применяемый алгоритм вне зависимости от использующих его объектов-клиентов.
"""
